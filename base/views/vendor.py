...
from utils.types import AdStatus, OrderStatus, UserType, CodeTypes, AdType
from utils.mixins import SellerRequiredMixin
from utils.helper import attach_ad_remaining_labels, attach_offer_remaining_labels
...
class VendorDashboardView(SellerRequiredMixin, View):
    def get(self, request):
        ...
        ads = SponsoredAd.objects.filter(product__tenant=vendor).annotate(
            db_clicks=Count('click_records')
        ).order_by('-start_date')
        orders = Order.objects.filter(tenant=vendor).order_by('-created_at')
        ad_clicks_count = ads.aggregate(total_clicks=Sum('db_clicks'))['total_clicks'] or 0
        ...
        stats = {
            ...
            'ad_clicks_count': ad_clicks_count,
        }
...
class VendorStoreView(View):
    def get(self, request, pk):
        ...
        products = Product.objects.filter(tenant=vendor).order_by('-created_at')
        ads = SponsoredAd.objects.filter(Q(product__tenant=vendor) & Q(start_date__lte=timezone.now()) & Q(end_date__gte=timezone.now()) & Q(ad_type=AdType.SECTION) & Q(status=AdStatus.ACTIVE)).order_by('-start_date')
        ...
        ads = attach_ad_remaining_labels(ads)
        offers = attach_offer_remaining_labels(offers)
        context = {
            'vendor': vendor,
            'products': products,
            'ads': ads,
...
class AdsListView(SellerRequiredMixin, View):
    def get(self, request):
        vendor = get_object_or_404(Vendor, user=request.user)
        query = request.GET.get('q')
        ads = SponsoredAd.objects.filter(product__tenant=vendor).select_related('product').annotate(
            db_clicks=Count('click_records')
        )
        if query:
