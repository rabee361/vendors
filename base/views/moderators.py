...
from ..models import Vendor, Product, Order, CustomUser, Buyer, StoreCategory, ContactMessage, SponsoredAd, Setting
from ..forms import CategoryForm, ModeratorForm, ModeratorLoginForm, ModeratorVendorForm, MessageForm, ModeratorSponsoredAdForm, ModeratorSettingForm
...
class ModeratorAdsView(ModeratorRequiredMixin, View):
    def get(self, request):
        query = request.GET.get('q')
        ads = SponsoredAd.objects.select_related('tenant', 'product').annotate(
            db_clicks=Count('click_records')
        ).order_by('-start_date')
        if query:
            ads = ads.filter(product__name__icontains=query)
        return render(request, 'moderator/ads.html', {'ads': ads})


class ModeratorAdUpdateView(ModeratorRequiredMixin, View):
    template_name = 'moderator/ad_form.html'

    def get(self, request, pk):
        instance = get_object_or_404(SponsoredAd.objects.select_related('tenant', 'product'), pk=pk)
        form = ModeratorSponsoredAdForm(instance=instance)
        return render(request, self.template_name, {'form': form, 'object': instance})

    def post(self, request, pk):
        instance = get_object_or_404(SponsoredAd.objects.select_related('tenant', 'product'), pk=pk)
        form = ModeratorSponsoredAdForm(request.POST, instance=instance)
        if form.is_valid():
            form.save()
            return redirect('moderator_ads')
        return render(request, self.template_name, {'form': form, 'object': instance})


class ModeratorSettingsView(ModeratorRequiredMixin, View):
    def get(self, request):
        query = request.GET.get('q')
        settings_qs = Setting.objects.all()
        if query:
            settings_qs = settings_qs.filter(name__icontains=query)
        settings_qs = settings_qs.order_by('name')
        return render(request, 'moderator/settings.html', {'settings': settings_qs})


class ModeratorSettingUpdateView(ModeratorRequiredMixin, View):
    template_name = 'moderator/setting_form.html'

    def get(self, request, pk):
        instance = get_object_or_404(Setting, pk=pk)
        form = ModeratorSettingForm(instance=instance)
        return render(request, self.template_name, {'form': form, 'object': instance})

    def post(self, request, pk):
        instance = get_object_or_404(Setting, pk=pk)
        form = ModeratorSettingForm(request.POST, instance=instance)
        if form.is_valid():
            form.save()
