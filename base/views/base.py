...
class HomeView(View):
    def get(self, request):
        ...
        ads = SponsoredAd.objects.select_related('tenant', 'product').filter(
            Q(ad_type=AdType.SECTION) & Q(start_date__lte=timezone.now()) & Q(end_date__gte=timezone.now()) & Q(status=AdStatus.ACTIVE)
        )[:3]
...
class OfferListView(View):
    def get(self, request):
        query = request.GET.get('q', '')
        offers = Offer.objects.filter(Q(start_date__lte=timezone.now()) & Q(end_date__gte=timezone.now()))
        if query:
...
class AdListView(View):
    def get(self, request):
        query = request.GET.get('q', '')
        ads = SponsoredAd.objects.select_related('tenant', 'product').filter(
            Q(ad_type=AdType.SECTION) & Q(start_date__lte=timezone.now()) & Q(end_date__gte=timezone.now())
        )
        if query:
...
class SponsoredAdClickRedirectView(View):
    def get(self, request, ad_id):
        today = timezone.now().date()
        ad = get_object_or_404(
            SponsoredAd.objects.select_related('product', 'tenant__user'),
            pk=ad_id,
        ...
        source_page = (request.GET.get('source') or '')[:50]
        SponsoredAdClickService(request).add(ad, source_page)

        return redirect('product', product_id=ad.product.pk)
