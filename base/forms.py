...
from utils.types import AdType, UserType
from utils.validators import SyrianPhoneValidator

from .models import ContactMessage, Coupon, CustomUser, OTPCode, Offer, Order, Product, ProductCategory, Setting, SponsoredAd, StoreCategory, Vendor
...
class ModeratorVendorForm(forms.ModelForm):
    ...
    def save(self, commit=True):
        ...
        vendor.user.is_active = self.cleaned_data.get('is_active', False)
        if commit:
            vendor.user.save(update_fields=['is_active'])
            vendor.save()
        return vendor

class ModeratorSponsoredAdForm(forms.ModelForm):
    class Meta:
        model = SponsoredAd
        fields = ['status','budget']


class ModeratorSettingForm(forms.ModelForm):
    class Meta:
        model = Setting
        fields = ['value']
        widgets = {
...
class SponsoredAdForm(forms.ModelForm):
    def __init__(self, *args, **kwargs):
        ...
        self.minimum_ad_budget = ad_settings['minimum_ad_budget']
        self.normal_ads_budget = ad_settings['normal_ads_budget']
        self.ad_click_cost = ad_settings['ad_click_cost']
        ...
        self.fields['budget'].widget.attrs.update({
            'placeholder': 'ميزانية الإعلان',
            'min': self.minimum_ad_budget,
            'step': 1,
        })

        if not self.is_bound:
            if self.instance and self.instance.pk:
                if self.instance.ad_type == AdType.BADGE:
                    self.initial['budget'] = self.normal_ads_budget
                else:
                    self.initial['budget'] = self.instance.budget or self.minimum_ad_budget
            else:
                self.initial['budget'] = self.minimum_ad_budget

    @property
    def ad_click_cost_display(self):
        if self.ad_click_cost == self.ad_click_cost.to_integral():
            return int(self.ad_click_cost)
        return self.ad_click_cost.normalize()
    ...
    def clean(self):
        ...
        if ad_type == AdType.BADGE:
            cleaned_data['budget'] = self.normal_ads_budget
            return cleaned_data

        if ad_type == AdType.SECTION:
            if budget is None:
                self.add_error('budget', 'أدخل ميزانية الإعلان.')
            elif budget <= 0:
                self.add_error('budget', 'ميزانية الإعلان يجب أن تكون أكبر من صفر.')
            elif budget < self.minimum_ad_budget:
                self.add_error('budget', f'الحد الأدنى لميزانية الإعلان هو {self.minimum_ad_budget}.')

        return cleaned_data

    class Meta:
        model = SponsoredAd
        fields = ['ad_type', 'product', 'budget', 'status', 'end_date']
        widgets = {
            'end_date': forms.DateInput(attrs={'type': 'date'}),
        }
